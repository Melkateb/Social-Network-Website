...
<html>
    ...
    <body>
        ...
        if (isset($_POST['submit-search'])) {
            ...
            $sql = "SELECT user.fname, user.lname, user.email, user.number, user.hometown, post.content FROM user LEFT JOIN post ON user.email = post.email WHERE user.fname = '$search' OR user.lname = '$search' OR user.email = '$search' OR post.content LIKE '%$search%' AND user.email <> '$user_email'";
            ...
            if ($queryResult > 0) {
                ...
                <h3>" . $row['fname'] . " " . $row['lname'] . "</h3>
                        ...
                        echo '
                    <a href="make_friend.php?make_friend=' . $email . ' ">' . $email . '</a>';
                    } else {
                        echo"<div>
                <h3>" . $row['fname'] . " " . $row['lname'] . "</h3>
                </div>";
